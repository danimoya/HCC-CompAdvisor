...
BEGIN
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE '
      CREATE TABLE T_COMPRESSION_LOG (
        -- Primary Key
        LOG_ID              NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,

        -- Log Level and Category
        LOG_LEVEL           VARCHAR2(10) NOT NULL,  -- DEBUG, INFO, WARNING, ERROR
        LOG_CATEGORY        VARCHAR2(30) DEFAULT ''GENERAL'',
        ...
        -- Constraints
        CONSTRAINT PK_COMPRESSION_LOG PRIMARY KEY (LOG_ID),
        CONSTRAINT CHK_LOG_LEVEL CHECK (
          LOG_LEVEL IN (''DEBUG'', ''INFO'', ''WARNING'', ''ERROR'', ''FATAL'')
        ),
        CONSTRAINT CHK_LOG_TIMESTAMP CHECK (LOG_TIMESTAMP IS NOT NULL)
      )';
...
BEGIN
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE '
      CREATE INDEX IDX_COMP_LOG_TIMESTAMP
      ON T_COMPRESSION_LOG(LOG_DATE DESC, LOG_LEVEL)';
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE '
      CREATE INDEX IDX_COMP_LOG_SOURCE
      ON T_COMPRESSION_LOG(PACKAGE_NAME, PROCEDURE_NAME, LOG_DATE DESC)';
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE '
      CREATE INDEX IDX_COMP_LOG_OBJECT
      ON T_COMPRESSION_LOG(OBJECT_OWNER, OBJECT_NAME, LOG_DATE DESC)';
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE '
      CREATE INDEX IDX_COMP_LOG_RUNS
      ON T_COMPRESSION_LOG(ADVISOR_RUN_ID, ANALYSIS_ID, EXECUTION_ID)';
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE '
      CREATE BITMAP INDEX IDX_COMP_LOG_LEVEL_BMP
      ON T_COMPRESSION_LOG(LOG_LEVEL)';
...
CREATE OR REPLACE PACKAGE BODY pkg_compression_log AS
  ...
  BEGIN
    ...
    BEGIN
      ...
      IF v_count = 0 THEN
        EXECUTE IMMEDIATE '
          CREATE TABLE T_COMPRESSION_LOG_ARCHIVE
          AS SELECT * FROM T_COMPRESSION_LOG WHERE 1=0';
      END IF;
    END;
