...
CREATE OR REPLACE PACKAGE BODY PKG_TEST_TABLE_GENERATOR AS
  ...
  BEGIN
    ...
    -- Insert data with good repetition (good for QUERY LOW) using EXECUTE IMMEDIATE
    EXECUTE IMMEDIATE 'INSERT INTO TEST_ADV_LOW_COMPRESSION SELECT ROWNUM as record_id, CASE MOD(ROWNUM, 10) WHEN 0 THEN ''North America'' WHEN 1 THEN ''Europe'' WHEN 2 THEN ''Asia'' WHEN 3 THEN ''South America'' WHEN 4 THEN ''Africa'' WHEN 5 THEN ''Australia'' WHEN 6 THEN ''Middle East'' WHEN 7 THEN ''Central America'' WHEN 8 THEN ''Eastern Europe'' ELSE ''Southeast Asia'' END as region, CASE MOD(ROWNUM, 50) WHEN 0 THEN ''United States'' WHEN 1 THEN ''Canada'' WHEN 2 THEN ''Mexico'' WHEN 3 THEN ''UK'' WHEN 4 THEN ''Germany'' WHEN 5 THEN ''France'' WHEN 6 THEN ''Japan'' WHEN 7 THEN ''China'' WHEN 8 THEN ''India'' WHEN 9 THEN ''Brazil'' ELSE ''Australia'' END as country, CASE MOD(ROWNUM, 20) WHEN 0 THEN ''New York'' WHEN 1 THEN ''Los Angeles'' WHEN 2 THEN ''Chicago'' WHEN 3 THEN ''London'' WHEN 4 THEN ''Paris'' WHEN 5 THEN ''Tokyo'' WHEN 6 THEN ''Delhi'' WHEN 7 THEN ''Sao Paulo'' WHEN 8 THEN ''Toronto'' WHEN 9 THEN ''Sydney'' ELSE ''Singapore'' END as city, CASE MOD(ROWNUM, 8) WHEN 0 THEN ''Sales'' WHEN 1 THEN ''Engineering'' WHEN 2 THEN ''Operations'' WHEN 3 THEN ''Finance'' WHEN 4 THEN ''Marketing'' WHEN 5 THEN ''HR'' WHEN 6 THEN ''Legal'' ELSE ''Support'' END as department, MOD(ROWNUM, 1000) + 1 as employee_id, ''Employee '' || LPAD(MOD(ROWNUM, 1000), 5, ''0'') as employee_name, 50000 + ROUND(DBMS_RANDOM.VALUE(0, 100000), 2) as salary, TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as hire_date, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''Excellent'' WHEN 1 THEN ''Good'' WHEN 2 THEN ''Average'' WHEN 3 THEN ''Below Average'' ELSE ''Poor'' END as performance, CASE MOD(ROWNUM, 4) WHEN 0 THEN ''Expert'' WHEN 1 THEN ''Advanced'' WHEN 2 THEN ''Intermediate'' ELSE ''Beginner'' END as skill_level, ''PROJ'' || LPAD(MOD(ROWNUM, 100), 5, ''0'') as project_code, ROUND(DBMS_RANDOM.VALUE(10, 50), 1) as hours_logged, 150 + ROUND(DBMS_RANDOM.VALUE(0, 200), 2) as billing_rate, (ROUND(DBMS_RANDOM.VALUE(10, 50), 1) * (150 + ROUND(DBMS_RANDOM.VALUE(0, 200), 2))) as revenue FROM dual CONNECT BY LEVEL <= 3000000';
  ...
  BEGIN
    ...
    -- Insert data with very high repetition (good for QUERY HIGH aggressive compression) using EXECUTE IMMEDIATE
    EXECUTE IMMEDIATE 'INSERT INTO TEST_ADV_HIGH_COMPRESSION SELECT ROWNUM as event_id, CASE MOD(ROWNUM, 6) WHEN 0 THEN ''ERROR'' WHEN 1 THEN ''WARNING'' WHEN 2 THEN ''INFO'' WHEN 3 THEN ''DEBUG'' WHEN 4 THEN ''CRITICAL'' ELSE ''NOTICE'' END as event_type, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''Database'' WHEN 1 THEN ''Application'' WHEN 2 THEN ''Network'' WHEN 3 THEN ''System'' ELSE ''Security'' END as event_category, CASE MOD(ROWNUM, 4) WHEN 0 THEN ''RESOLVED'' WHEN 1 THEN ''PENDING'' WHEN 2 THEN ''IN_PROGRESS'' ELSE ''CLOSED'' END as event_status, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''CRITICAL'' WHEN 1 THEN ''HIGH'' WHEN 2 THEN ''MEDIUM'' WHEN 3 THEN ''LOW'' ELSE ''INFO'' END as severity_level, CASE MOD(ROWNUM, 8) WHEN 0 THEN ''APP_SERVER_01'' WHEN 1 THEN ''APP_SERVER_02'' WHEN 2 THEN ''DB_SERVER_01'' WHEN 3 THEN ''DB_SERVER_02'' WHEN 4 THEN ''WEB_SERVER_01'' WHEN 5 THEN ''CACHE_SERVER_01'' WHEN 6 THEN ''LOAD_BALANCER_01'' ELSE ''MONITORING_SERVER'' END as source_system, ''Location_'' || LPAD(MOD(ROWNUM, 20), 3, ''0'') as source_location, SYSTIMESTAMP - NUMTODSINTERVAL(MOD(ROWNUM, 87600), ''HOUR'') as event_time, TRUNC(SYSDATE - MOD(ROWNUM, 730)) as event_date, ''Event description for record '' || ROWNUM as description, ''Detailed information: System='' || MOD(ROWNUM, 8) || '', Code='' || MOD(ROWNUM, 1000) as details, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''Restarted service'' WHEN 1 THEN ''Cleared cache'' WHEN 2 THEN ''Applied patch'' WHEN 3 THEN ''Reconfigured system'' ELSE ''Escalated to team'' END as resolution, ''tag1,tag2,tag'' || MOD(ROWNUM, 10) as tags, ''env='' || CASE MOD(ROWNUM, 3) WHEN 0 THEN ''prod'' WHEN 1 THEN ''staging'' ELSE ''dev'' END || '',version=1.0,build='' || MOD(ROWNUM, 100) as metadata FROM dual CONNECT BY LEVEL <= 5000000';
  ...
  BEGIN
    ...
    -- Insert large volume of query-optimized data (good for QUERY LOW HCC) using EXECUTE IMMEDIATE
    EXECUTE IMMEDIATE 'INSERT INTO TEST_HCC_QUERY_LOW SELECT ROWNUM as transaction_id, MOD(ROWNUM, 5000) + 1 as customer_id, MOD(ROWNUM, 2000) + 1 as order_id, ''SKU'' || LPAD(MOD(ROWNUM, 500), 5, ''0'') as sku, CASE MOD(ROWNUM, 10) WHEN 0 THEN ''Electronics'' WHEN 1 THEN ''Clothing'' WHEN 2 THEN ''Home & Garden'' WHEN 3 THEN ''Sports'' WHEN 4 THEN ''Books'' WHEN 5 THEN ''Toys'' WHEN 6 THEN ''Beauty'' WHEN 7 THEN ''Health'' WHEN 8 THEN ''Food'' ELSE ''Other'' END as product_line, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''Online'' WHEN 1 THEN ''Retail'' WHEN 2 THEN ''Wholesale'' WHEN 3 THEN ''Direct'' ELSE ''Partner'' END as channel, MOD(ROWNUM, 100) + 1 as store_id, MOD(ROWNUM, 50) + 1 as warehouse_id, TRUNC(DBMS_RANDOM.VALUE(1, 100)) as quantity_sold, ROUND(DBMS_RANDOM.VALUE(10, 1000), 2) as unit_price, TRUNC(DBMS_RANDOM.VALUE(1, 100)) * ROUND(DBMS_RANDOM.VALUE(10, 1000), 2) as sale_amount, ROUND(DBMS_RANDOM.VALUE(5, 500), 2) as cost_amount, ROUND(DBMS_RANDOM.VALUE(10, 50), 2) as margin_percent, TRUNC(SYSDATE - MOD(ROWNUM, 730)) as transaction_date, CASE MOD(ROWNUM, 7) WHEN 0 THEN ''North America'' WHEN 1 THEN ''Europe'' WHEN 2 THEN ''Asia Pacific'' WHEN 3 THEN ''Latin America'' WHEN 4 THEN ''Middle East'' WHEN 5 THEN ''Africa'' ELSE ''Oceania'' END as region, CASE MOD(ROWNUM, 50) WHEN 0 THEN ''USA'' WHEN 1 THEN ''Canada'' WHEN 2 THEN ''Mexico'' WHEN 3 THEN ''UK'' WHEN 4 THEN ''Germany'' WHEN 5 THEN ''Japan'' WHEN 6 THEN ''China'' WHEN 7 THEN ''India'' WHEN 8 THEN ''Brazil'' ELSE ''Other'' END as country, CASE MOD(ROWNUM, 4) WHEN 0 THEN ''Premium'' WHEN 1 THEN ''Standard'' WHEN 2 THEN ''Economy'' ELSE ''VIP'' END as customer_segment, CASE MOD(ROWNUM, 6) WHEN 0 THEN ''Credit Card'' WHEN 1 THEN ''Debit Card'' WHEN 2 THEN ''PayPal'' WHEN 3 THEN ''Bank Transfer'' WHEN 4 THEN ''Check'' ELSE ''Cash'' END as payment_method, CASE WHEN MOD(ROWNUM, 20) = 0 THEN ''SUMMER20'' ELSE NULL END as promo_code, CASE WHEN MOD(ROWNUM, 20) = 0 THEN 20 ELSE 0 END as discount_applied FROM dual CONNECT BY LEVEL <= 10000000';
  ...
  BEGIN
    ...
    -- Insert high-volume repetitive log data (good for QUERY HIGH HCC) using EXECUTE IMMEDIATE
    EXECUTE IMMEDIATE 'INSERT INTO TEST_HCC_QUERY_HIGH SELECT ROWNUM as log_id, SYSTIMESTAMP - NUMTODSINTERVAL(MOD(ROWNUM, 1440), ''MINUTE'') as timestamp, TRUNC(SYSDATE - MOD(ROWNUM, 30)) as log_date, LPAD(TRUNC(MOD(ROWNUM, 1440) / 60), 2, ''0'') || '':'' || LPAD(MOD(ROWNUM, 60), 2, ''0'') as log_time, CASE MOD(ROWNUM, 20) WHEN 0 THEN ''SERVER_PROD_01'' WHEN 1 THEN ''SERVER_PROD_02'' WHEN 2 THEN ''SERVER_PROD_03'' WHEN 3 THEN ''SERVER_STAGE_01'' WHEN 4 THEN ''SERVER_DEV_01'' ELSE ''SERVER_LOAD_'' || LPAD(MOD(ROWNUM, 10), 2, ''0'') END as server_name, CASE MOD(ROWNUM, 10) WHEN 0 THEN ''DATABASE'' WHEN 1 THEN ''APPLICATION'' WHEN 2 THEN ''WEB_SERVER'' WHEN 3 THEN ''API_SERVER'' WHEN 4 THEN ''CACHE'' WHEN 5 THEN ''QUEUE'' WHEN 6 THEN ''SCHEDULER'' WHEN 7 THEN ''MONITOR'' WHEN 8 THEN ''SECURITY'' ELSE ''OTHER'' END as service_name, TRUNC(DBMS_RANDOM.VALUE(1, 10000)) as process_id, TRUNC(DBMS_RANDOM.VALUE(1, 1000)) as thread_id, CASE MOD(ROWNUM, 6) WHEN 0 THEN ''DEBUG'' WHEN 1 THEN ''INFO'' WHEN 2 THEN ''WARN'' WHEN 3 THEN ''ERROR'' WHEN 4 THEN ''FATAL'' ELSE ''TRACE'' END as log_level, ''Log entry for process '' || TRUNC(DBMS_RANDOM.VALUE(1, 10000)) as log_message, CASE WHEN MOD(ROWNUM, 100) < 5 THEN ''ERR_'' || LPAD(MOD(ROWNUM, 100), 3, ''0'') ELSE NULL END as error_code, CASE WHEN MOD(ROWNUM, 100) < 5 THEN ''Error occurred in module X'' ELSE NULL END as error_message, CASE WHEN MOD(ROWNUM, 100) < 5 THEN ''at module.function:'' || MOD(ROWNUM, 1000) ELSE NULL END as stack_trace, ''USER_'' || LPAD(MOD(ROWNUM, 1000), 5, ''0'') as user_id, ''SESSION_'' || LPAD(MOD(ROWNUM, 5000), 5, ''0'') as session_id, ''REQ_'' || LPAD(MOD(ROWNUM, 10000), 8, ''0'') as request_id, ''CORR_'' || LPAD(MOD(ROWNUM, 1000), 5, ''0'') as correlation_id, CASE MOD(ROWNUM, 3) WHEN 0 THEN ''PRODUCTION'' WHEN 1 THEN ''STAGING'' ELSE ''DEVELOPMENT'' END as environment, ''2.0.'' || LPAD(MOD(ROWNUM, 100), 2, ''0'') as version, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''com.company.app.core'' WHEN 1 THEN ''com.company.app.service'' WHEN 2 THEN ''com.company.app.dao'' WHEN 3 THEN ''com.company.app.util'' ELSE ''com.company.app.web'' END as component FROM dual CONNECT BY LEVEL <= 20000000';
  ...
  BEGIN
    ...
    -- Insert historical/archival data (good for ARCHIVE LOW HCC) using EXECUTE IMMEDIATE
    EXECUTE IMMEDIATE 'INSERT INTO TEST_HCC_ARCHIVE_LOW SELECT ROWNUM as archive_id, 2000 + TRUNC((ROWNUM - 1) / 100000) as fiscal_year, ''Q'' || (MOD((ROWNUM - 1), 4) + 1) as fiscal_quarter, MOD((ROWNUM - 1), 12) + 1 as fiscal_month, CASE MOD((ROWNUM - 1), 4) + 1 WHEN 1 THEN ''Q1 - Jan/Feb/Mar'' WHEN 2 THEN ''Q2 - Apr/May/Jun'' WHEN 3 THEN ''Q3 - Jul/Aug/Sep'' ELSE ''Q4 - Oct/Nov/Dec'' END as quarter_name, CASE MOD((ROWNUM - 1), 12) + 1 WHEN 1 THEN ''January'' WHEN 2 THEN ''February'' WHEN 3 THEN ''March'' WHEN 4 THEN ''April'' WHEN 5 THEN ''May'' WHEN 6 THEN ''June'' WHEN 7 THEN ''July'' WHEN 8 THEN ''August'' WHEN 9 THEN ''September'' WHEN 10 THEN ''October'' WHEN 11 THEN ''November'' ELSE ''December'' END as month_name, TRUNC(SYSDATE - MOD(ROWNUM, 7300)) as date_key, CASE MOD(ROWNUM, 7) WHEN 0 THEN ''North America'' WHEN 1 THEN ''Europe'' WHEN 2 THEN ''Asia'' WHEN 3 THEN ''Latin America'' WHEN 4 THEN ''Africa'' WHEN 5 THEN ''Middle East'' ELSE ''Oceania'' END as region, CASE MOD(ROWNUM, 50) WHEN 0 THEN ''USA'' WHEN 1 THEN ''Canada'' WHEN 2 THEN ''UK'' WHEN 3 THEN ''Germany'' WHEN 4 THEN ''France'' WHEN 5 THEN ''Japan'' WHEN 6 THEN ''China'' WHEN 7 THEN ''India'' WHEN 8 THEN ''Brazil'' ELSE ''Other'' END as country, CASE MOD(ROWNUM, 10) WHEN 0 THEN ''CA'' WHEN 1 THEN ''NY'' WHEN 2 THEN ''TX'' ELSE ''Other'' END as state, CASE MOD(ROWNUM, 20) WHEN 0 THEN ''New York'' WHEN 1 THEN ''Los Angeles'' WHEN 2 THEN ''Chicago'' WHEN 3 THEN ''Houston'' WHEN 4 THEN ''Phoenix'' ELSE ''Other City'' END as city, CASE MOD(ROWNUM, 3) WHEN 0 THEN ''Consumer'' WHEN 1 THEN ''Enterprise'' ELSE ''Government'' END as customer_type, CASE MOD(ROWNUM, 8) WHEN 0 THEN ''Electronics'' WHEN 1 THEN ''Clothing'' WHEN 2 THEN ''Food'' WHEN 3 THEN ''Furniture'' WHEN 4 THEN ''Appliances'' WHEN 5 THEN ''Beauty'' WHEN 6 THEN ''Books'' ELSE ''Other'' END as product_category, ROUND(DBMS_RANDOM.VALUE(100000, 10000000), 2) as revenue, ROUND(DBMS_RANDOM.VALUE(50000, 5000000), 2) as cost, ROUND(DBMS_RANDOM.VALUE(50000, 5000000), 2) as gross_profit, TRUNC(DBMS_RANDOM.VALUE(1000, 100000)) as units_sold, ROUND(DBMS_RANDOM.VALUE(10, 1000), 2) as avg_price, ROUND(DBMS_RANDOM.VALUE(5, 30), 2) as market_share, ROUND(DBMS_RANDOM.VALUE(-10, 25), 2) as growth_rate, ''ARCHIVED'' as status, TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as archive_date, ''BATCH_LOADER'' as created_by, ''Historical data for archive '' || ROWNUM as notes FROM dual CONNECT BY LEVEL <= 5000000';
  ...
  BEGIN
    ...
    -- Insert massive archival dataset with high repetition (good for ARCHIVE HIGH HCC) using EXECUTE IMMEDIATE
    EXECUTE IMMEDIATE 'INSERT INTO TEST_HCC_ARCHIVE_HIGH SELECT ROWNUM as record_id, TO_CHAR(TRUNC(SYSDATE - MOD(ROWNUM, 7300)), ''YYYY-MM'') as year_month, 2000 + TRUNC((ROWNUM - 1) / 100000) as year, MOD((ROWNUM - 1), 12) + 1 as month, TRUNC(MOD((ROWNUM - 1), 52)) + 1 as week_num, TRUNC(MOD((ROWNUM - 1), 365)) + 1 as day_num, TRUNC(SYSDATE - MOD(ROWNUM, 7300)) as record_date, CASE MOD(ROWNUM, 20) WHEN 0 THEN ''LEGACY_SYSTEM_01'' WHEN 1 THEN ''LEGACY_SYSTEM_02'' WHEN 2 THEN ''ERP_SYSTEM'' WHEN 3 THEN ''CRM_SYSTEM'' WHEN 4 THEN ''DW_SYSTEM'' ELSE ''ARCHIVE_SYSTEM'' END as source_system, CASE MOD(ROWNUM, 8) WHEN 0 THEN ''TRANSACTION'' WHEN 1 THEN ''INVOICE'' WHEN 2 THEN ''PAYMENT'' WHEN 3 THEN ''JOURNAL'' WHEN 4 THEN ''REPORT'' WHEN 5 THEN ''AUDIT'' WHEN 6 THEN ''LOG'' ELSE ''OTHER'' END as data_type, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''ACTIVE'' WHEN 1 THEN ''ARCHIVED'' WHEN 2 THEN ''OBSOLETE'' WHEN 3 THEN ''DELETED'' ELSE ''RETAINED'' END as status, CASE MOD(ROWNUM, 6) WHEN 0 THEN ''CRITICAL'' WHEN 1 THEN ''HIGH'' WHEN 2 THEN ''MEDIUM'' WHEN 3 THEN ''LOW'' WHEN 4 THEN ''INFO'' ELSE ''DEBUG'' END as severity, TRUNC(DBMS_RANDOM.VALUE(1, 5)) as priority, ''USER_'' || LPAD(MOD(ROWNUM, 50), 3, ''0'') as assigned_to, CASE MOD(ROWNUM, 10) WHEN 0 THEN ''Finance'' WHEN 1 THEN ''Operations'' WHEN 2 THEN ''Sales'' WHEN 3 THEN ''IT'' WHEN 4 THEN ''HR'' WHEN 5 THEN ''Legal'' WHEN 6 THEN ''Compliance'' WHEN 7 THEN ''Audit'' WHEN 8 THEN ''Archive'' ELSE ''General'' END as department, ''CC'' || LPAD(MOD(ROWNUM, 100), 4, ''0'') as cost_center, ''PROJ'' || LPAD(MOD(ROWNUM, 1000), 5, ''0'') as project_id, ''BDG'' || LPAD(MOD(ROWNUM, 100), 4, ''0'') as budget_code, ROUND(DBMS_RANDOM.VALUE(100, 100000), 2) as amount, CASE MOD(ROWNUM, 5) WHEN 0 THEN ''EUR'' WHEN 1 THEN ''GBP'' WHEN 2 THEN ''JPY'' ELSE ''USD'' END as currency, ''Archived record description '' || ROWNUM as description, ''Long form description for archival record number '' || ROWNUM || '' containing detailed information about the transaction, including context, notes, and supplementary details for compliance and audit purposes.'' as long_description, ''REF'' || LPAD(MOD(ROWNUM, 100000), 6, ''0'') as reference_id, ''ITEM_'' || LPAD(MOD(ROWNUM, 100), 4, ''0'') || '','' || ''ITEM_'' || LPAD(MOD(ROWNUM + 1, 100), 4, ''0'') as related_items, ''Archived on '' || TO_CHAR(SYSDATE, ''YYYY-MM-DD'') as comments, CASE MOD(ROWNUM, 4) WHEN 0 THEN ''APPROVED'' ELSE ''ARCHIVED'' END as approval_status, ''ADMIN'' as approved_by, TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as approval_date, TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as effective_date, TRUNC(SYSDATE - MOD(ROWNUM, 3600)) + 365 as expiry_date, TRUNC(SYSDATE - MOD(ROWNUM, 30)) as last_modified FROM dual CONNECT BY LEVEL <= 10000000';
