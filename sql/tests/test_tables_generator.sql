...
CREATE OR REPLACE PACKAGE BODY PKG_TEST_TABLE_GENERATOR AS
  ...
  BEGIN
    ...
    SELECT
      ROWNUM as id,
      MOD(ROWNUM, 1000) + 1 as customer_id,
      MOD(ROWNUM, 500) + 1 as order_id,
      'PROD' || LPAD(MOD(ROWNUM, 100), 5, '0') as product_code,
      CASE MOD(ROWNUM, 5)
        ...
        ELSE 'Sports'
      END as category,
      TRUNC(SYSDATE - MOD(ROWNUM, 365)) as order_date,
      ROUND(DBMS_RANDOM.VALUE(10, 1000), 2) as amount,
      TRUNC(DBMS_RANDOM.VALUE(1, 50)) as quantity,
      CASE MOD(ROWNUM, 4)
        WHEN 0 THEN 'PENDING'
        WHEN 1 THEN 'SHIPPED'
        ...
        ELSE 'CANCELLED'
      END as status,
      'Order for customer ' || MOD(ROWNUM, 1000) as description,
      TRUNC(SYSDATE - MOD(ROWNUM, 30)) as created_date,
      TRUNC(SYSDATE) as updated_date
    FROM dual
  ...
  BEGIN
    ...
    SELECT
      ...
      MOD(ROWNUM, 500) + 1 as user_id,
      'SESSION_' || LPAD(MOD(ROWNUM, 100), 5, '0') as session_id,
      CASE MOD(ROWNUM, 3)
        WHEN 0 THEN 'DEPOSIT'
      ...
      CASE MOD(ROWNUM, 2)
        ...
        ELSE 'PENDING'
      END as status,
      SYSTIMESTAMP - NUMTODSINTERVAL(MOD(ROWNUM, 1440), 'MINUTE') as transaction_time,
      SYSTIMESTAMP as updated_time,
      'Transaction for user ' || MOD(ROWNUM, 500) as notes,
      CASE WHEN MOD(ROWNUM, 1000) = 0 THEN 1 ELSE NULL END as error_code,
      TRUNC(DBMS_RANDOM.VALUE(0, 3)) as retry_count
    FROM dual
  ...
  BEGIN
    ...
    SELECT
      ...
      CASE MOD(ROWNUM, 10)
        ...
        WHEN 6 THEN 'Middle East'
        WHEN 7 THEN 'Central America'
      ...
      CASE MOD(ROWNUM, 8)
        ...
        ELSE 'Support'
      END as department,
      MOD(ROWNUM, 1000) + 1 as employee_id,
      'Employee ' || LPAD(MOD(ROWNUM, 1000), 5, '0') as employee_name,
      50000 + ROUND(DBMS_RANDOM.VALUE(0, 100000), 2) as salary,
      TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as hire_date,
      CASE MOD(ROWNUM, 5)
      ...
      CASE MOD(ROWNUM, 4)
        ...
        ELSE 'Beginner'
      END as skill_level,
      'PROJ' || LPAD(MOD(ROWNUM, 100), 5, '0') as project_code,
      ROUND(DBMS_RANDOM.VALUE(10, 50), 1) as hours_logged,
      150 + ROUND(DBMS_RANDOM.VALUE(0, 200), 2) as billing_rate,
      (ROUND(DBMS_RANDOM.VALUE(10, 50), 1) * (150 + ROUND(DBMS_RANDOM.VALUE(0, 200), 2))) as revenue
    FROM dual
  ...
  BEGIN
    ...
    SELECT
      ...
      CASE MOD(ROWNUM, 8)
        WHEN 0 THEN 'APP_SERVER_01'
        WHEN 1 THEN 'APP_SERVER_02'
        WHEN 2 THEN 'DB_SERVER_01'
        WHEN 3 THEN 'DB_SERVER_02'
        WHEN 4 THEN 'WEB_SERVER_01'
        WHEN 5 THEN 'CACHE_SERVER_01'
        WHEN 6 THEN 'LOAD_BALANCER_01'
        ELSE 'MONITORING_SERVER'
      END as source_system,
      'Location_' || LPAD(MOD(ROWNUM, 20), 3, '0') as source_location,
      SYSTIMESTAMP - NUMTODSINTERVAL(MOD(ROWNUM, 87600), 'HOUR') as event_time,
      TRUNC(SYSDATE - MOD(ROWNUM, 730)) as event_date,
      'Event description for record ' || ROWNUM as description,
      'Detailed information: System=' || MOD(ROWNUM, 8) || ', Code=' || MOD(ROWNUM, 1000) as details,
      CASE MOD(ROWNUM, 5)
        WHEN 0 THEN 'Restarted service'
        WHEN 1 THEN 'Cleared cache'
        WHEN 2 THEN 'Applied patch'
        WHEN 3 THEN 'Reconfigured system'
        ELSE 'Escalated to team'
      END as resolution,
      'tag1,tag2,tag' || MOD(ROWNUM, 10) as tags,
      'env=' || CASE MOD(ROWNUM, 3) WHEN 0 THEN 'prod' WHEN 1 THEN 'staging' ELSE 'dev' END ||
      ',version=1.0,build=' || MOD(ROWNUM, 100) as metadata
    FROM dual
  ...
  BEGIN
    ...
    SELECT
      ROWNUM as transaction_id,
      MOD(ROWNUM, 5000) + 1 as customer_id,
      MOD(ROWNUM, 2000) + 1 as order_id,
      'SKU' || LPAD(MOD(ROWNUM, 500), 5, '0') as sku,
      CASE MOD(ROWNUM, 10)
        WHEN 0 THEN 'Electronics'
        WHEN 1 THEN 'Clothing'
      ...
      CASE MOD(ROWNUM, 5)
        ...
        ELSE 'Partner'
      END as channel,
      MOD(ROWNUM, 100) + 1 as store_id,
      MOD(ROWNUM, 50) + 1 as warehouse_id,
      TRUNC(DBMS_RANDOM.VALUE(1, 100)) as quantity_sold,
      ROUND(DBMS_RANDOM.VALUE(10, 1000), 2) as unit_price,
      TRUNC(DBMS_RANDOM.VALUE(1, 100)) * ROUND(DBMS_RANDOM.VALUE(10, 1000), 2) as sale_amount,
      ROUND(DBMS_RANDOM.VALUE(5, 500), 2) as cost_amount,
      ROUND(DBMS_RANDOM.VALUE(10, 50), 2) as margin_percent,
      TRUNC(SYSDATE - MOD(ROWNUM, 730)) as transaction_date,
      CASE MOD(ROWNUM, 7)
        WHEN 0 THEN 'North America'
        WHEN 1 THEN 'Europe'
      ...
      CASE WHEN MOD(ROWNUM, 20) = 0 THEN 'SUMMER20' ELSE NULL END as promo_code,
      CASE WHEN MOD(ROWNUM, 20) = 0 THEN 20 ELSE 0 END as discount_applied
    FROM dual
  ...
  BEGIN
    ...
    SELECT
      ROWNUM as log_id,
      SYSTIMESTAMP - NUMTODSINTERVAL(MOD(ROWNUM, 1440), 'MINUTE') as timestamp,
      TRUNC(SYSDATE - MOD(ROWNUM, 30)) as log_date,
      LPAD(TRUNC(MOD(ROWNUM, 1440) / 60), 2, '0') || ':' ||
      LPAD(MOD(ROWNUM, 60), 2, '0') as log_time,
      CASE MOD(ROWNUM, 20)
        WHEN 0 THEN 'SERVER_PROD_01'
        WHEN 1 THEN 'SERVER_PROD_02'
        WHEN 2 THEN 'SERVER_PROD_03'
        WHEN 3 THEN 'SERVER_STAGE_01'
        WHEN 4 THEN 'SERVER_DEV_01'
        ELSE 'SERVER_LOAD_' || LPAD(MOD(ROWNUM, 10), 2, '0')
      END as server_name,
      CASE MOD(ROWNUM, 10)
        WHEN 0 THEN 'DATABASE'
        WHEN 1 THEN 'APPLICATION'
        WHEN 2 THEN 'WEB_SERVER'
        WHEN 3 THEN 'API_SERVER'
        WHEN 4 THEN 'CACHE'
        WHEN 5 THEN 'QUEUE'
        ...
        ELSE 'OTHER'
      END as service_name,
      TRUNC(DBMS_RANDOM.VALUE(1, 10000)) as process_id,
      TRUNC(DBMS_RANDOM.VALUE(1, 1000)) as thread_id,
      CASE MOD(ROWNUM, 6)
        WHEN 0 THEN 'DEBUG'
        WHEN 1 THEN 'INFO'
        WHEN 2 THEN 'WARN'
        ...
        ELSE 'TRACE'
      END as log_level,
      'Log entry for process ' || TRUNC(DBMS_RANDOM.VALUE(1, 10000)) as log_message,
      CASE WHEN MOD(ROWNUM, 100) < 5 THEN 'ERR_' || LPAD(MOD(ROWNUM, 100), 3, '0') ELSE NULL END as error_code,
      CASE WHEN MOD(ROWNUM, 100) < 5 THEN 'Error occurred in module X' ELSE NULL END as error_message,
      CASE WHEN MOD(ROWNUM, 100) < 5 THEN 'at module.function:' || MOD(ROWNUM, 1000) ELSE NULL END as stack_trace,
      'USER_' || LPAD(MOD(ROWNUM, 1000), 5, '0') as user_id,
      'SESSION_' || LPAD(MOD(ROWNUM, 5000), 5, '0') as session_id,
      'REQ_' || LPAD(MOD(ROWNUM, 10000), 8, '0') as request_id,
      'CORR_' || LPAD(MOD(ROWNUM, 1000), 5, '0') as correlation_id,
      CASE MOD(ROWNUM, 3)
        WHEN 0 THEN 'PRODUCTION'
        WHEN 1 THEN 'STAGING'
        ELSE 'DEVELOPMENT'
      END as environment,
      '2.0.' || LPAD(MOD(ROWNUM, 100), 2, '0') as version,
      CASE MOD(ROWNUM, 5)
        WHEN 0 THEN 'com.company.app.core'
        WHEN 1 THEN 'com.company.app.service'
        WHEN 2 THEN 'com.company.app.dao'
        WHEN 3 THEN 'com.company.app.util'
        ELSE 'com.company.app.web'
      END as component
    FROM dual
  ...
  BEGIN
    ...
    SELECT
      ...
      2000 + TRUNC((ROWNUM - 1) / 100000) as fiscal_year,
      'Q' || (MOD((ROWNUM - 1), 4) + 1) as fiscal_quarter,
      MOD((ROWNUM - 1), 12) + 1 as fiscal_month,
      CASE MOD((ROWNUM - 1), 4) + 1
        WHEN 1 THEN 'Q1 - Jan/Feb/Mar'
        WHEN 2 THEN 'Q2 - Apr/May/Jun'
        WHEN 3 THEN 'Q3 - Jul/Aug/Sep'
        ELSE 'Q4 - Oct/Nov/Dec'
      END as quarter_name,
      CASE MOD((ROWNUM - 1), 12) + 1
        WHEN 1 THEN 'January'
        ...
        ELSE 'December'
      END as month_name,
      TRUNC(SYSDATE - MOD(ROWNUM, 7300)) as date_key,
      CASE MOD(ROWNUM, 7)
        WHEN 0 THEN 'North America'
        WHEN 1 THEN 'Europe'
        WHEN 2 THEN 'Asia'
      ...
      CASE MOD(ROWNUM, 50)
        ...
        ELSE 'Other'
      END as country,
      CASE MOD(ROWNUM, 10) WHEN 0 THEN 'CA' WHEN 1 THEN 'NY' WHEN 2 THEN 'TX' ELSE 'Other' END as state,
      CASE MOD(ROWNUM, 20)
        WHEN 0 THEN 'New York'
        WHEN 1 THEN 'Los Angeles'
        WHEN 2 THEN 'Chicago'
      ...
      CASE MOD(ROWNUM, 8)
        ...
        WHEN 4 THEN 'Appliances'
        WHEN 5 THEN 'Beauty'
        WHEN 6 THEN 'Books'
        ELSE 'Other'
      END as product_category,
      ROUND(DBMS_RANDOM.VALUE(100000, 10000000), 2) as revenue,
      ROUND(DBMS_RANDOM.VALUE(50000, 5000000), 2) as cost,
      ROUND(DBMS_RANDOM.VALUE(50000, 5000000), 2) as gross_profit,
      TRUNC(DBMS_RANDOM.VALUE(1000, 100000)) as units_sold,
      ROUND(DBMS_RANDOM.VALUE(10, 1000), 2) as avg_price,
      ROUND(DBMS_RANDOM.VALUE(5, 30), 2) as market_share,
      ROUND(DBMS_RANDOM.VALUE(-10, 25), 2) as growth_rate,
      'ARCHIVED' as status,
      TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as archive_date,
      'BATCH_LOADER' as created_by,
      'Historical data for archive ' || ROWNUM as notes
    FROM dual
    CONNECT BY LEVEL <= 5000000;
  ...
  BEGIN
    ...
    SELECT
      ROWNUM as record_id,
      TO_CHAR(TRUNC(SYSDATE - MOD(ROWNUM, 7300)), 'YYYY-MM') as year_month,
      2000 + TRUNC((ROWNUM - 1) / 100000) as year,
      MOD((ROWNUM - 1), 12) + 1 as month,
      TRUNC(MOD((ROWNUM - 1), 52)) + 1 as week_num,
      TRUNC(MOD((ROWNUM - 1), 365)) + 1 as day_num,
      TRUNC(SYSDATE - MOD(ROWNUM, 7300)) as record_date,
      CASE MOD(ROWNUM, 20)
        WHEN 0 THEN 'LEGACY_SYSTEM_01'
        WHEN 1 THEN 'LEGACY_SYSTEM_02'
        WHEN 2 THEN 'ERP_SYSTEM'
        WHEN 3 THEN 'CRM_SYSTEM'
        WHEN 4 THEN 'DW_SYSTEM'
        ELSE 'ARCHIVE_SYSTEM'
      END as source_system,
      CASE MOD(ROWNUM, 8)
      ...
      'USER_' || LPAD(MOD(ROWNUM, 50), 3, '0') as assigned_to,
      CASE MOD(ROWNUM, 10)
        WHEN 0 THEN 'Finance'
        ...
        ELSE 'General'
      END as department,
      'CC' || LPAD(MOD(ROWNUM, 100), 4, '0') as cost_center,
      'PROJ' || LPAD(MOD(ROWNUM, 1000), 5, '0') as project_id,
      'BDG' || LPAD(MOD(ROWNUM, 100), 4, '0') as budget_code,
      ROUND(DBMS_RANDOM.VALUE(100, 100000), 2) as amount,
      CASE MOD(ROWNUM, 5) WHEN 0 THEN 'EUR' WHEN 1 THEN 'GBP' WHEN 2 THEN 'JPY' ELSE 'USD' END as currency,
      'Archived record description ' || ROWNUM as description,
      'Long form description for archival record number ' || ROWNUM ||
      ' containing detailed information about the transaction, including context, ' ||
      'notes, and supplementary details for compliance and audit purposes.' as long_description,
      'REF' || LPAD(MOD(ROWNUM, 100000), 6, '0') as reference_id,
      'ITEM_' || LPAD(MOD(ROWNUM, 100), 4, '0') || ',' ||
      'ITEM_' || LPAD(MOD(ROWNUM + 1, 100), 4, '0') as related_items,
      'Archived on ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD') as comments,
      CASE MOD(ROWNUM, 4) WHEN 0 THEN 'APPROVED' ELSE 'ARCHIVED' END as approval_status,
      'ADMIN' as approved_by,
      TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as approval_date,
      TRUNC(SYSDATE - MOD(ROWNUM, 3650)) as effective_date,
      TRUNC(SYSDATE - MOD(ROWNUM, 3600)) + 365 as expiry_date,
      TRUNC(SYSDATE - MOD(ROWNUM, 30)) as last_modified
    FROM dual
    CONNECT BY LEVEL <= 10000000;
