 ...
 *
 * DESCRIPTION:
 *   Centralized schema for multi-database compression analysis and execution
 *   tracking. Extends the base schema with target database registration and
 *   per-database scoping for all analysis/history tables.
 *
 *   Strategy tables (T_COMPRESSION_STRATEGIES, T_STRATEGY_RULES) remain global
 *   and shared across all target databases.
 *
 *   Analysis and history tables gain a DATABASE_ID foreign key to
 *   T_TARGET_DATABASES for efficient per-database filtering.
 *
 * SUPPORTED COMPRESSION TYPES:
 *   - BASIC (ROW STORE COMPRESS BASIC)
 *   - OLTP (ROW STORE COMPRESS ADVANCED)
 *   - QUERY LOW (COLUMN STORE COMPRESS FOR QUERY LOW / HCC)
 *   - QUERY HIGH (COLUMN STORE COMPRESS FOR QUERY HIGH / HCC)
 *   - ARCHIVE LOW (COLUMN STORE COMPRESS FOR ARCHIVE LOW / HCC)
 *   - ARCHIVE HIGH (COLUMN STORE COMPRESS FOR ARCHIVE HIGH / HCC)
 *
 * USAGE:
...
PROMPT ================================================================================
PROMPT Creating HCC Compression Advisor Centralized Server Schema (Oracle 23c Free)
PROMPT ================================================================================

-- ============================================================================
-- SECTION 1: TARGET DATABASE REGISTRATION TABLE
-- ============================================================================

PROMPT
PROMPT Creating target database registration table...

-- ----------------------------------------------------------------------------
-- Table: T_TARGET_DATABASES
-- Purpose: Registry of all target databases monitored by the central advisor
-- ----------------------------------------------------------------------------
CREATE TABLE T_TARGET_DATABASES (
    -- Primary Key
    DATABASE_ID             NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,
    ...
    ENVIRONMENT             VARCHAR2(30) DEFAULT 'PRODUCTION',
    -- Status
    ...
    PLATFORM_TYPE           VARCHAR2(30) DEFAULT 'STANDARD',
    CONNECTION_MODE         VARCHAR2(10) DEFAULT 'NORMAL',  -- NORMAL or SYSDBA
    -- Audit
    ...
    -- Constraints
    CONSTRAINT PK_TARGET_DATABASES PRIMARY KEY (DATABASE_ID),
    CONSTRAINT UNQ_DATABASE_NAME UNIQUE (DATABASE_NAME),
    CONSTRAINT CHK_TARGET_ENVIRONMENT CHECK (
        ENVIRONMENT IN ('PRODUCTION', 'DEV', 'TEST', 'UAT', 'STAGING')
    ),
    CONSTRAINT CHK_TARGET_IS_ACTIVE CHECK (IS_ACTIVE IN ('Y', 'N')),
    CONSTRAINT CHK_TARGET_PLATFORM_TYPE CHECK (
        PLATFORM_TYPE IN ('STANDARD', 'EXADATA')
    )
);
COMMENT ON TABLE T_TARGET_DATABASES IS 'Registry of all target databases monitored by the centralized compression advisor';
COMMENT ON COLUMN T_TARGET_DATABASES.DATABASE_NAME IS 'Unique logical name for the target database';
COMMENT ON COLUMN T_TARGET_DATABASES.PASSWORD_ENCRYPTED IS 'Encrypted database password - never store plaintext';
COMMENT ON COLUMN T_TARGET_DATABASES.ENVIRONMENT IS 'Deployment environment: PRODUCTION, DEV, TEST, UAT, STAGING';
COMMENT ON COLUMN T_TARGET_DATABASES.PLATFORM_TYPE IS 'Oracle platform type: STANDARD or EXADATA (enables HCC)';
...
-- ============================================================================
-- SECTION 3: ANALYSIS RESULT TABLES (Per-database via DATABASE_ID)
-- ============================================================================
PROMPT
...
CREATE TABLE T_COMPRESSION_ANALYSIS (
    ...
    CONSTRAINT FK_COMP_ANALYSIS_DB FOREIGN KEY (DATABASE_ID)
        REFERENCES T_TARGET_DATABASES(DATABASE_ID),
    ...
    CONSTRAINT CHK_ADVISABLE_COMPRESSION CHECK (
        ADVISABLE_COMPRESSION IN ('NONE', 'BASIC', 'OLTP', 'QUERY LOW', 'QUERY HIGH', 'ARCHIVE LOW', 'ARCHIVE HIGH')
    ),
...
COMMENT ON TABLE T_COMPRESSION_ANALYSIS IS 'Compression analysis results for tables and IOTs (centralized, per-database)';
COMMENT ON COLUMN T_COMPRESSION_ANALYSIS.DATABASE_ID IS 'Reference to the target database this analysis belongs to';
...
CREATE INDEX IDX_COMP_ANALYSIS_DB ON T_COMPRESSION_ANALYSIS(
    DATABASE_ID
);
...
CREATE INDEX IDX_COMP_ANALYSIS_DB_OBJECT ON T_COMPRESSION_ANALYSIS(
    DATABASE_ID, OWNER, OBJECT_NAME
);
CREATE INDEX IDX_COMP_ANALYSIS_OBJECT ON T_COMPRESSION_ANALYSIS(
    OWNER, OBJECT_NAME
);
CREATE INDEX IDX_COMP_ANALYSIS_DB_HOTNESS ON T_COMPRESSION_ANALYSIS(
    DATABASE_ID, HOTNESS_SCORE
);
CREATE INDEX IDX_COMP_ANALYSIS_HOTNESS ON T_COMPRESSION_ANALYSIS(
    HOTNESS_SCORE
);
CREATE INDEX IDX_COMP_ANALYSIS_DB_RUN ON T_COMPRESSION_ANALYSIS(
    DATABASE_ID, ADVISOR_RUN_ID, ANALYSIS_DATE DESC
);
...
CREATE TABLE T_INDEX_COMPRESSION_ANALYSIS (
    ...
    CONSTRAINT FK_INDEX_ANALYSIS_DB FOREIGN KEY (DATABASE_ID)
        REFERENCES T_TARGET_DATABASES(DATABASE_ID),
    CONSTRAINT CHK_INDEX_ADVISABLE_COMPRESSION CHECK (
        ADVISABLE_COMPRESSION IN ('NONE', 'PREFIX', 'ADVANCED_LOW', 'ADVANCED_HIGH')
    )
);
COMMENT ON TABLE T_INDEX_COMPRESSION_ANALYSIS IS 'Index compression analysis results (centralized, per-database)';
COMMENT ON COLUMN T_INDEX_COMPRESSION_ANALYSIS.DATABASE_ID IS 'Reference to the target database this analysis belongs to';
CREATE INDEX IDX_INDEX_COMP_DB ON T_INDEX_COMPRESSION_ANALYSIS(
    DATABASE_ID
);
CREATE INDEX IDX_INDEX_COMP_DB_NAME ON T_INDEX_COMPRESSION_ANALYSIS(
    DATABASE_ID, INDEX_OWNER, INDEX_NAME
);
CREATE INDEX IDX_INDEX_COMP_NAME ON T_INDEX_COMPRESSION_ANALYSIS(
    INDEX_OWNER, INDEX_NAME
);
CREATE INDEX IDX_INDEX_COMP_DB_TABLE ON T_INDEX_COMPRESSION_ANALYSIS(
    DATABASE_ID, TABLE_OWNER, TABLE_NAME
);
...
CREATE TABLE T_LOB_COMPRESSION_ANALYSIS (
    ...
    CONSTRAINT FK_LOB_ANALYSIS_DB FOREIGN KEY (DATABASE_ID)
        REFERENCES T_TARGET_DATABASES(DATABASE_ID),
...
COMMENT ON TABLE T_LOB_COMPRESSION_ANALYSIS IS 'LOB compression analysis results (centralized, per-database)';
COMMENT ON COLUMN T_LOB_COMPRESSION_ANALYSIS.DATABASE_ID IS 'Reference to the target database this analysis belongs to';
CREATE INDEX IDX_LOB_COMP_DB ON T_LOB_COMPRESSION_ANALYSIS(
    DATABASE_ID
);
CREATE INDEX IDX_LOB_COMP_DB_TABLE ON T_LOB_COMPRESSION_ANALYSIS(
    DATABASE_ID, TABLE_OWNER, TABLE_NAME, COLUMN_NAME
);
CREATE INDEX IDX_LOB_COMP_TABLE ON T_LOB_COMPRESSION_ANALYSIS(
    TABLE_OWNER, TABLE_NAME, COLUMN_NAME
);
-- ============================================================================
-- SECTION 4: EXECUTION HISTORY TABLES (Per-database via DATABASE_ID)
-- ============================================================================
PROMPT
...
CREATE TABLE T_COMPRESSION_HISTORY (
    ...
    CONSTRAINT FK_COMP_HISTORY_DB FOREIGN KEY (DATABASE_ID)
        REFERENCES T_TARGET_DATABASES(DATABASE_ID),
...
-- At most one open (QUEUED / IN_PROGRESS) row per segment of a target, so two
-- users can't queue or run the same segment twice. Closed rows have an all-NULL
-- key and are not indexed. Same index as patch 20260924-history-unique-open-segment.
...
CREATE INDEX IDX_HISTORY_DB_OBJECT ON T_COMPRESSION_HISTORY(
    DATABASE_ID, OWNER, OBJECT_NAME, START_TIME DESC
);
CREATE INDEX IDX_HISTORY_OBJECT ON T_COMPRESSION_HISTORY(
    OWNER, OBJECT_NAME, START_TIME DESC
);
CREATE INDEX IDX_HISTORY_DB_STATUS ON T_COMPRESSION_HISTORY(
    DATABASE_ID, OPERATION_STATUS, START_TIME DESC
);
...
-- ============================================================================
-- SECTION 5: ADVISOR RUN TRACKING (Per-database via DATABASE_ID)
-- ============================================================================
PROMPT
...
CREATE TABLE T_ADVISOR_RUN (
    ...
    CONSTRAINT FK_ADVISOR_RUN_DB FOREIGN KEY (DATABASE_ID)
        REFERENCES T_TARGET_DATABASES(DATABASE_ID),
...
COMMENT ON TABLE T_ADVISOR_RUN IS 'Analysis execution sessions and metadata (centralized, per-database)';
COMMENT ON COLUMN T_ADVISOR_RUN.DATABASE_ID IS 'Reference to the target database this run belongs to';
-- Indexes
CREATE INDEX IDX_ADVISOR_RUN_DB ON T_ADVISOR_RUN(
    DATABASE_ID
);
CREATE INDEX IDX_ADVISOR_RUN_DB_TIME ON T_ADVISOR_RUN(
    DATABASE_ID, START_TIME DESC, RUN_STATUS
);
CREATE INDEX IDX_ADVISOR_RUN_TIME ON T_ADVISOR_RUN(
    START_TIME DESC, RUN_STATUS
);
CREATE INDEX IDX_ADVISOR_RUN_DB_SCHEMA ON T_ADVISOR_RUN(
    DATABASE_ID, SCHEMA_FILTER, START_TIME DESC
);
...
-- ============================================================================
-- SECTION 6: FOREIGN KEY CONSTRAINTS (Deferred for circular dependencies)
-- ============================================================================
PROMPT
...
PROMPT
PROMPT ================================================================================
PROMPT Centralized Schema Creation Summary
PROMPT ================================================================================
...
WHERE object_name LIKE 'T_%COMPRESS%'
   ...
   OR object_name LIKE 'IDX_%HISTORY%'
   OR object_name LIKE 'IDX_%ADVISOR%'
...
PROMPT
PROMPT ================================================================================
PROMPT Centralized schema creation completed successfully!
PROMPT ================================================================================
PROMPT
PROMPT Next Steps:
PROMPT 1. Review the created objects above
PROMPT 2. Seed default strategy data: @02_central_seed.sql
PROMPT 3. Install PL/SQL packages: @03_central_packages.sql
PROMPT 4. Create reporting views: @04_central_views.sql
PROMPT 5. Run test suite: @05_central_test.sql
PROMPT ================================================================================
