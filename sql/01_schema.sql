...
CREATE TABLE T_COMPRESSION_ANALYSIS (
    ...
    DATA_AGE_DAYS           NUMBER GENERATED ALWAYS AS (
        TRUNC(SYSDATE - NVL(LAST_ANALYZED, SYSDATE))
    ) VIRTUAL,
...
CREATE INDEX IDX_COMP_ANALYSIS_RECOMMENDATION ON T_COMPRESSION_ANALYSIS(
    ADVISABLE_COMPRESSION, PROJECTED_SAVINGS_MB DESC
);
...
CREATE BITMAP INDEX IDX_COMP_ANALYSIS_CATEGORY_BMP ON T_COMPRESSION_ANALYSIS(
    HOTNESS_CATEGORY
);
