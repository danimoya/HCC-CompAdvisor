...
def show_analysis_config():
    ...
    with st.expander("What is the hotness score?", expanded=False):
        st.markdown(
            "The **hotness score (0–100)** measures how actively an object is used, "
            "on an absolute scale: a table gets the same score whatever else is in the scan.\n\n"
            "It blends two rates:\n"
            "- **Writes per day** — DML counted by `DBA_TAB_MODIFICATIONS` divided by the "
            "days since statistics were last gathered, cross-checked against *db block "
            "changes* from `V$SEGMENT_STATISTICS` (not reset by a stats gather, so a busy "
            "table analyzed last night does not drop to 0). Also weighted by churn "
            "relative to `NUM_ROWS`. Roughly: 1K/day ≈ 50, 10K/day ≈ 67, 1M+/day = 100.\n"
            "- **Block reads per day** — logical + physical reads from "
            "`V$SEGMENT_STATISTICS` (plus AWR `DBA_HIST_SEG_STAT` when the Diagnostics "
            "Pack is acknowledged under Admin). Reads alone score at most 60: read-mostly "
            "data is still a good `QUERY` HCC candidate.\n\n"
            "- **0–24 · COLD** — idle. *Ideal* for `ARCHIVE` HCC.\n"
            "- **25–49 · COOL** — mostly read, little DML. `QUERY HIGH` is typical.\n"
            "- **50–74 · WARM** — regular reads and/or moderate DML. `QUERY LOW` or `OLTP`.\n"
            "- **75–100 · HOT** — frequent DML. Aggressive HCC risks row migration and "
            "decompress-on-update overhead — prefer `OLTP`, or leave uncompressed.\n\n"
            "Each recommendation's reason shows the measured rates and which sources were "
            "readable. If the advisor user cannot read these views the score falls back to "
            "what is available (grant `SELECT_CATALOG_ROLE` for the full picture).\n\n"
